    ...
    final class WooCommerce_Gateways_Country_Limiter {
        ...
        function init() {
            ...
            if ( !empty( $_GET['section'] ) ) {
                $this->current_section = sanitize_title( $_GET['section'] );
            }

            foreach ( $payment_gateways as $id => $gateway ) {
                $title = empty( $gateway->method_title ) ? ucfirst( $gateway->id ) : $gateway->method_title;
                $section_id = version_compare( WC()->version, '2.6.0', '>=' ) ? $id : 'wc_gateway_' . $id;
                $this->sections[$section_id] = array( 'title' => esc_html( $title ), 'id' => $id );
            }

            if ( is_admin() && !empty( $this->current_section ) ) {
            ...
            if ( is_admin() && !empty( $this->current_section ) ) {
                if ( isset( $this->sections[$this->current_section] ) ) {
                    $gateway_id = $this->sections[$this->current_section]['id'];
                    wc_enqueue_js( "
        ...
        function country_options_output() {
            ...
            if ( !empty( $this->current_section ) && !empty( $this->sections[$this->current_section] ) ):
                ...
                <table class="form-table">
                    <tr valign="top">
                        <th scope="row" class="titledesc">
                            <label
                                for="<?php echo $gateway_id ?>_country_options"><?php _e( 'Country availability', 'wcpgpl' ) ?></label>
                            <img class="help_tip"
                                 data-tip="<?php esc_attr_e( 'Configure per country availability for this payment gateway', 'wcpgpl' ) ?>"
                        ...
                        <td class="forminp forminp-radio">
                            <fieldset>
                                <ul>
                                    <li>
                                        <input type="hidden" name="woocommerce_gateways_country_limiter"
                                               value="<?php echo $gateway_id ?>"/>
                                        <label>
                                            <input class="input-text regular-input " type="radio"
                                                   name="<?php echo $gateway_id ?>_option" value="all" checked="checked"
                                                   style="width: auto;" <?php checked( 'all', $this->settings[$gateway_id]['option'] ); ?> /><?php _e( 'Available for all countries', 'wcpgpl' ) ?>
                                        </label>
                                    ...
                                    <li>
                                        <label>
                                            <input class="input-text regular-input " type="radio"
                                                   name="<?php echo $gateway_id ?>_option" value="all_except"
                                                   style="width: auto;" <?php checked( 'all_except', $this->settings[$gateway_id]['option'] ); ?> /><?php _e( 'All countries except selected', 'wcpgpl' ) ?>
                                        </label>
                                    ...
                                    <li>
                                        <label>
                                            <input class="input-text regular-input " type="radio"
                                                   name="<?php echo $gateway_id ?>_option" value="selected"
                                                   style="width: auto;" <?php checked( 'selected', $this->settings[$gateway_id]['option'] ); ?> /><?php _e( 'Only selected countries', 'wcpgpl' ) ?>
                                        </label>
                                    </li>
                                </ul>

                                <div id="pgcl_countries_list" style="display:none;">
                                    <select name="<?php echo $gateway_id ?>_countries[]" multiple="multiple"
                                            title="<?php esc_attr_e( 'Country', 'wcpgpl' ) ?>" class="chosen_select"
                                            data-placeholder="<?php esc_attr_e( 'Select countries&hellip;', 'wcpgpl' ); ?>">
                                        <?php foreach ( WC()->countries->countries as $code => $country ): ?>
                                            <option value="<?php echo esc_attr( $code ) ?>"
                                                    <?php if ( in_array( $code, (array)$this->settings[$gateway_id]['countries'] ) ): ?>selected="selected"<?php endif; ?>><?php echo esc_html( $country ); ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
